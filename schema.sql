...
create or replace function public.create_battle(
  p_name text, p_start_date date, p_end_date date, p_carryover_cash numeric, p_players jsonb
)
...
begin
  ...
  if p_carryover_cash < 0 then raise exception '繰越余剰金は0円以上にしてください'; end if;
  insert into public.battles(name,start_date,end_date,carryover_cash)
  values(trim(p_name),p_start_date,p_end_date,coalesce(p_carryover_cash,0)) returning * into v_battle;
  for v_player in select * from jsonb_array_elements(p_players) loop
    ...
    values(v_battle.id,trim(v_player->>'nickname'),trim(v_player->>'security_code'),trim(v_player->>'security_name'),
      (v_player->>'purchase_price')::numeric,v_shares,0,v_player->>'color',(v_player->>'display_order')::smallint);
  end loop;
...
grant execute on function public.create_battle(text,date,date,numeric,jsonb) to anon, authenticated;
...
create or replace function public.update_battle(
  p_share_token text,p_name text,p_start_date date,p_end_date date,p_carryover_cash numeric,p_players jsonb
)
returns void language plpgsql security definer set search_path=public as $$
...
begin
  ...
  if p_carryover_cash < 0 then raise exception '繰越余剰金は0円以上にしてください'; end if;
  if jsonb_array_length(p_players) <> 4 then raise exception '参加者は4人必要です'; end if;
  update public.battles set name=trim(p_name),start_date=p_start_date,end_date=p_end_date,carryover_cash=coalesce(p_carryover_cash,0) where id=v_battle_id;
  for v_player in select * from jsonb_array_elements(p_players) loop
...
grant execute on function public.update_battle(text,text,date,date,numeric,jsonb) to anon, authenticated;
