-- Supabase SQL Editorで、このファイル全体を一度実行してください。
...
begin
  ...
  for v_player in select * from jsonb_array_elements(p_players) loop
    insert into public.players(battle_id,nickname,security_code,security_name,purchase_price,color,display_order)
    values(v_battle.id,trim(v_player->>'nickname'),trim(v_player->>'security_code'),trim(v_player->>'security_name'),(v_player->>'purchase_price')::numeric,v_player->>'color',(v_player->>'display_order')::smallint);
  end loop;
