-- 株価成長バトル Ver.2.0 更新用SQL
-- このファイル全体をSupabase SQL Editorの新しいタブへ貼り付け、1回実行してください。
...
do $$ begin
  alter table public.players add constraint players_shares_1_to_10 check (shares_owned >= 1 and shares_owned <= 10 and trunc(shares_owned) = shares_owned);
exception when duplicate_object then null; end $$;
...
-- 旧版で参加者ごとに入っていた余剰金を、対戦全体の1枠へ合計して移します。
update public.battles b
set carryover_cash = x.total_cash
from (
  select battle_id, sum(coalesce(carryover_cash, 0)) as total_cash
  from public.players group by battle_id
) x
where b.id = x.battle_id and b.carryover_cash = 0 and x.total_cash > 0;

update public.players set carryover_cash = 0 where carryover_cash <> 0;

-- 旧シグネチャを削除して、Ver.2.0版へ統一します。
...
begin
  ...
  insert into public.battles(name,start_date,end_date,carryover_cash,rules)
  values(trim(p_name),p_start_date,p_end_date,coalesce(p_carryover_cash,0),coalesce(p_rules,''))
  returning * into v_battle;

  for v_player in select * from jsonb_array_elements(p_players) loop
    v_shares := coalesce((v_player->>'shares_owned')::numeric,1);
    if v_shares < 1 or v_shares > 10 or trunc(v_shares) <> v_shares then raise exception '所有株数は1〜10株の整数にしてください'; end if;
    ...
    values(v_battle.id,trim(v_player->>'nickname'),trim(v_player->>'security_code'),trim(v_player->>'security_name'),(v_player->>'purchase_price')::numeric,v_shares,0,v_player->>'color',(v_player->>'display_order')::smallint);
  end loop;
...
begin
  ...
  update public.battles set name=trim(p_name),start_date=p_start_date,end_date=p_end_date,carryover_cash=coalesce(p_carryover_cash,0),rules=coalesce(p_rules,'') where id=v_battle_id;

  for v_player in select * from jsonb_array_elements(p_players) loop
    v_player_id := (v_player->>'id')::uuid;
    v_shares := coalesce((v_player->>'shares_owned')::numeric,1);
    if v_shares < 1 or v_shares > 10 or trunc(v_shares) <> v_shares then raise exception '所有株数は1〜10株の整数にしてください'; end if;
    update public.players set nickname=trim(v_player->>'nickname'),security_code=trim(v_player->>'security_code'),security_name=trim(v_player->>'security_name'),purchase_price=(v_player->>'purchase_price')::numeric,shares_owned=v_shares,carryover_cash=0,color=v_player->>'color',display_order=(v_player->>'display_order')::smallint where id=v_player_id and battle_id=v_battle_id;
...
select jsonb_build_object(
  ...
  'carryover_cash',b.carryover_cash,'rules',b.rules,'created_at',b.created_at,
...
begin
  ...
  if jsonb_array_length(p_prices) <> v_active_count then raise exception '未利確の参加者全員の終値を入力してください'; end if;

  for v_item in select * from jsonb_array_elements(p_prices) loop
    v_player_id := (v_item->>'player_id')::uuid;
    if not exists(select 1 from public.players where id=v_player_id and battle_id=v_battle_id and is_sold=false) then raise exception '利確済み、または不正な参加者が含まれています'; end if;
...
create or replace function public.set_player_sale(p_share_token text,p_player_id uuid,p_sold_date date,p_sold_price numeric)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  select id into v_battle_id from public.battles where share_token=p_share_token;
  if v_battle_id is null then raise exception '対戦が見つかりません'; end if;
  if p_sold_date > current_date then raise exception '未来の日付では利確できません'; end if;
...
create or replace function public.clear_player_sale(p_share_token text,p_player_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
grant execute on function public.create_battle(text,date,date,numeric,text,jsonb) to anon,authenticated;
grant execute on function public.update_battle(text,text,date,date,numeric,text,jsonb) to anon,authenticated;
