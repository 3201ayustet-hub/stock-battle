(() => {
  ...
  window.StockBattle = {
    ...
    dateText(date) { return new Intl.DateTimeFormat('ja-JP', { year: 'numeric', month: 'numeric', day: 'numeric' }).format(new Date(`${date}T00:00:00`)); },
    shortDate(date) { return new Intl.DateTimeFormat('ja-JP', { month: 'numeric', day: 'numeric' }).format(new Date(`${date}T00:00:00`)); },
    ...
    setLoading(button, loading, label = '保存中…') {
      if (!button) return;
      if (loading) { button.dataset.original = button.textContent; button.textContent = label; button.disabled = true; }
    ...
    bindNav() {
      document.querySelectorAll('[data-page]').forEach(link => { link.href = this.battleUrl(link.dataset.page); });
    },
