(() => {
  ...
  function renderPlayers(players = []) {
    ...
    players.forEach((p, i) => { if (form.elements[`shares_${i}`]) form.elements[`shares_${i}`].value = String(Math.min(10, Math.max(1, Math.round(Number(p.shares_owned || 1))))); });
  }
  async function renderSaved() {
    ...
    try {
      ...
      list.innerHTML = items.map(item => `<article class="saved-battle-item"><div><strong>${SB.escapeHtml(item.name)}</strong><small>${item.start_date ? SB.dateText(item.start_date) : ''}</small></div><div class="saved-actions"><a class="primary button-link compact-button" href="${SB.battleUrl('index.html', item.share_token)}">開く</a><a class="secondary button-link compact-button" href="${SB.battleUrl('setup.html', item.share_token)}">設定</a><button type="button" class="danger compact-button delete-saved-battle" data-token="${SB.escapeHtml(item.share_token)}" data-name="${SB.escapeHtml(item.name)}">削除</button></div></article>`).join('');
    } catch (error) {
      section.classList.remove('hidden');
  ...
  function payload() {
    ...
    return { fd, start, end, initialAssets, carryoverCash, rules: String(fd.get('rules') || '').trim(), players: COLORS.map((_, i) => ({ id: fd.get(`id_${i}`) || undefined, nickname: String(fd.get(`nickname_${i}`)).trim(), security_code: String(fd.get(`code_${i}`)).trim(), security_name: String(fd.get(`security_${i}`)).trim(), purchase_price: Number(fd.get(`purchase_${i}`)), shares_owned: Number(fd.get(`shares_${i}`)), color: fd.get(`color_${i}`), display_order: i + 1 })) };
  }
  async function save(event) {
    ...
    try {
      const p = payload();
      const args = { p_name: p.fd.get('battle_name').trim(), p_start_date: p.start, p_end_date: p.end, p_initial_assets: p.initialAssets, p_carryover_cash: p.carryoverCash, p_rules: p.rules, p_players: p.players };
      if (editingBattle) {
  ...
  async function saveSaleEdit(event) {
    ...
    try {
      const { error } = await SB.db.rpc('set_player_sale', { p_share_token: SB.token, p_player_id: document.getElementById('sale-edit-player-id').value, p_sold_date: document.getElementById('sale-edit-date').value, p_sold_price: Number(document.getElementById('sale-edit-price').value) });
      if (error) throw error;
      document.getElementById('sale-edit-dialog').close(); editingBattle = await SB.loadBattle(); renderPlayers(editingBattle.players); SB.showMessage('利確情報を修正しました。');
  ...
  async function clearSale(playerId) {
    ...
    try {
      ...
      if (error) throw error;
      editingBattle = await SB.loadBattle(); renderPlayers(editingBattle.players); SB.showMessage('利確を取り消しました。');
  ...
  async function init() {
    ...
    await renderSaved(); renderPlayers(); form.classList.remove('hidden'); form.start_date.value = SB.today();
    if (SB.token) {
      try {
        editingBattle = await SB.loadBattle();
        document.getElementById('setup-title').textContent = '対戦内容を変更';
      ...
      } catch (error) { SB.showMessage(error.message || '読み込めませんでした。', 'error'); }
    }
