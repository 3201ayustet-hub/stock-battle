(() => {
  ...
  function renderRanking(rows) {
    rows.sort((a, b) => b.currentAsset - a.currentAsset);
    document.getElementById('ranking-cards').innerHTML = rows.map((row, index) => {
      ...
      return `<details class="ranking-card ${row.sold ? 'is-sold' : ''}" style="--player-color:${SB.escapeHtml(p.color || '#3b82f6')}">
        <summary>
          <span class="rank-mark">${rankLabel(index)}</span>
          <span class="member-name"><strong>${SB.escapeHtml(p.nickname)}</strong>${row.sold ? '<span class="sold-badge">✓ 利確済み</span>' : ''}</span>
          <span class="asset-block"><small>現在資産</small><strong>${SB.yen(row.currentAsset)}</strong></span>
          <span class="profit-block"><small>今期損益</small><strong class="${row.profit >= 0 ? 'positive' : 'negative'}">${SB.signedYen(row.profit)}</strong></span>
          <span class="growth-block"><small>今期成長率</small><strong class="${row.growth >= 0 ? 'positive' : 'negative'}">${SB.pct(row.growth)}</strong></span>
  ...
  function renderChart() {
    ...
    chart = new Chart(document.getElementById('growth-chart'), {
      type: 'line',
      data: { labels: dates.map(SB.shortDate), datasets: battle.players.map(player => ({ label: player.nickname, data: dates.map(date => chartValue(player, date)), borderColor: player.color, backgroundColor: player.color, borderWidth: 2.5, tension: 0, pointRadius: 2, pointHoverRadius: 5, spanGaps: true })) },
      options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, plugins: { legend: { position: 'bottom', labels: { color: '#d7dce5', usePointStyle: true, boxWidth: 7 } }, tooltip: { callbacks: { label: ctx => `${ctx.dataset.label}: ${SB.pct(ctx.parsed.y)}` } } }, scales: { x: { ticks: { color: '#8f98a8', maxTicksLimit: 7 }, grid: { display: false } }, y: { ticks: { color: '#8f98a8', callback: value => `${value}%` }, grid: { color: ctx => ctx.tick.value === 0 ? '#596273' : '#242a34' } } } }
    });
  }
