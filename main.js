  ...
  const endLabelPlugin = {
    ...
    afterDatasetsDraw(chartInstance) {
      ...
      chartInstance.data.datasets.forEach((dataset, datasetIndex) => {
        ...
        while (idx >= 0 && (dataset.data[idx] === null || dataset.data[idx] === undefined)) idx--;
        if (idx < 0 || !meta.data[idx]) return;
        const point = meta.data[idx];
        const value = Number(dataset.data[idx]);
        ctx.font = '10px "SFMono-Regular", "SF Mono", Menlo, Consolas, monospace';
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ctx.fillStyle = dataset.borderColor;
        ctx.fillText(SB.pct(value), chartInstance.chartArea.right - 4, point.y - 8);
      });
      ctx.restore();
    }
  };
